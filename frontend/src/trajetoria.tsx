...
function CommandArgs({ n, i, nodes, update_node }: { n: TrajetoriaNode, i: number, nodes: TrajetoriaNode[], update_node: any }) {
  ...
  switch (n.command.type) {
    case CommandType.Iniciar:
      return (
        <>
          <label> <input title="x (mm)" type="number" step="0.2" value={n.command.x} onChange={(e: any) => update_x(parseFloat(e.target.value))} /> </label>
          <label> <input title="y (mm)" type="number" step="0.2" value={n.command.y} onChange={(e: any) => update_y(parseFloat(e.target.value))} /> </label>
        </>
      );
    ...
    <>
      ...
      <div className="trajetoria">
        ...
        <div className="controls">
          ...
          <button disabled={!status.running && (!status.connected || !status.calibrated || !status.gcode_loaded || is_dirty)} onClick={toggle_play_pause}>
            {status.running ?
